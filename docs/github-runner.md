...
Bastion uses a self-hosted runner to execute local Kubernetes cluster smoke tests.
...
```powershell
./config.cmd --url https://github.com/nirjxr26/Bastion-IAM --token <TOKEN> --labels self-hosted,Bastion-runner,docker --unattended --replace
```
