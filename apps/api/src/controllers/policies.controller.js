...
exports.createPolicy = async (req, res, next) => {
    try {
        ...
        if (existing)
            return res.status(400).json({ success: false, error: 'Policy name must be unique' });
        ...
        if (
            ...
            resources.length === 0 ||
            !resources.every((r) => typeof r === 'string' && r.trim().length > 0)
        ) {
...
exports.simulatePolicy = async (req, res, next) => {
    try {
        ...
        if (!userId || !action || !resource) {
            return res
                .status(400)
                .json({ success: false, error: 'userId, action, resource are required' });
        }
