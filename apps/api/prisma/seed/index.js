...
const groupMembers = [
    ...
    [
        [0, 1, 8, 9],
        [roleIdByName.SecurityAuditor, roleIdByName.ReadOnlyAccess],
    ],
...
async function seedRoles() {
    ...
    await prisma.policy.createMany({
        data: policies.map((p) => ({
            ...
            effect: toPrismaPolicyEffect(p.effect),
            actions: p.actions,
            resources: p.resources,
            conditions: { seededBy: 'production-seed', type: p.type },
            isSystem: p.type === 'aws_managed',
    ...
    const rolePolicyRows = policies.flatMap((p) =>
        p.attachedToRoles.map((roleId) => ({ id: uuidv4(), roleId, policyId: p.id })),
    );
    const userGroupRows = groups.flatMap((g) =>
        g.memberIds.map((userId, idx) => ({
            id: uuidv4(),
            userId,
            groupId: g.id,
    ...
    const groupRoleRows = groups.flatMap((g) =>
        g.attachedRoleIds.map((roleId) => ({
            id: uuidv4(),
            groupId: g.id,
            roleId,
...
async function seedDatabase() {
    ...
    const summary = {
        ...
        userRoles: joinRows.userRoleRows.length,
        rolePolicies: joinRows.rolePolicyRows.length,
        userGroups: joinRows.userGroupRows.length,
