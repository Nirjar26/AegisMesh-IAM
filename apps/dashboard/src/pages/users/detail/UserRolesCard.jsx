                ...
                {roles.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {roles.map((r) => (
                            <div
                                key={r.id}
                                className="bg-[#ffffff] border border-[#d0d7e8] rounded-lg p-4"
                            >
                                <h4 className="font-bold text-slate-200 mb-1">{r.name}</h4>
                                <p className="text-xs text-[#7a87a8] line-clamp-2">
                    ...
                    <div className="bg-[#ffffff] border border-[#d0d7e8] rounded-lg overflow-hidden">
                        <table className="w-full text-sm">
                            <thead className="bg-[#f4f6fb]">
                                <tr>
                                    <th className="px-4 py-2 font-medium text-[#3a4560] text-left">
                                        Action
                                    </th>
                                    <th className="px-4 py-2 font-medium text-[#3a4560] text-center">
                                        Resource
                                    </th>
                                    <th className="px-4 py-2 font-medium text-[#3a4560] text-center">
                                        Effect
                            ...
                            <tbody className="divide-y divide-slate-800">
                                {permissions.map((p) => (
                                    <tr key={`${p.action}-${p.resource}`}>
                                        <td className="px-4 py-2 text-[#3a4560] font-mono text-xs text-left">
                                            {p.action}
                                        </td>
                                        <td className="px-4 py-2 text-[#7a87a8] font-mono text-xs text-center">
                                            {p.resource}
                                        </td>
                                        <td className="px-4 py-2 text-center">
                                            <span
                                                className={`inline-flex text-[10px] px-2 py-0.5 rounded-full font-bold uppercase ${p.effect === 'ALLOW' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}
                                            >
                                                {p.effect}
                                            </span>
                                        </td>
