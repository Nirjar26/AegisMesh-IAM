...
const InputField = forwardRef(
    (
        { label, type = 'text', error, icon: Icon, className = '', id, name, required, ...props },
        ref,
    ) => {
        ...
        const inputId =
            id || name || `field-${(label || 'input').toLowerCase().replace(/[^a-z0-9]+/g, '-')}`;
        ...
        return (
            <div className={`space-y-1.5 ${className}`}>
                {label && (
                    <label
                        htmlFor={inputId}
                        className="block text-sm font-medium text-aws-text-dim"
                    >
                        {label}
                        {required ? (
