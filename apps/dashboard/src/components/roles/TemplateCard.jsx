...
export default function TemplateCard({ template, onSelect }) {
    ...
    const hasBadge = Boolean(template.badge);

    const visiblePermissions = template.permissions.slice(0, 3);

    const visibleRestrictions = template.restrictions.slice(0, 2);

    const extraPermissions = template.permissions.length - 3;
    ...
    return (
        ...
        >
            ...
            <h3 className="mt-3 text-left text-base font-bold text-slate-900">{template.name}</h3>
            ...
            <div className="mt-4 text-left">
                ...
                {visiblePermissions.map((permission) => (
                    <div key={permission} className="mt-1 flex items-start gap-1.5">
                ...
                {extraPermissions > 0 && (
                    <div className="ml-4 mt-1 text-xs text-slate-400">+{extraPermissions} more</div>
                )}
            </div>

            <div className="mt-2 text-left">
                {visibleRestrictions.map((restriction) => (
                    <div key={restriction} className="mt-1 flex items-start gap-1.5">
                        <X size={11} className="mt-0.5 flex-shrink-0 text-red-400" />
            ...
            <div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-3">
                <span className="text-xs text-slate-400">
                    {template.estimatedPolicies} policies
                </span>
...
TemplateCard.propTypes = {
    ...
    template: PropTypes.shape({
        ...
        estimatedPolicies: PropTypes.oneOfType([PropTypes.string, PropTypes.number]).isRequired,
