...
This directory contains the complete GitOps-driven infrastructure for Bastion.

## Architecture Overview

Bastion is deployed as a cloud-native application on a **k3s** cluster. We utilize **ArgoCD** for continuous synchronization and **Kustomize** for environment-specific configuration (overlays).
...
2. Build local images:
    ```bash
    docker build -t Bastion-backend:local ./backend
